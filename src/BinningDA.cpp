...
#endif
{
	...
	for (dataTypeNSeq i=0; i<nRows; i++) { //scorri sulle righe calcolate
		rowIdx=firstRow+i;
		row_set_id=reads_to_set[rowIdx%n]; //0 se non sta in nessun set, k>0 se sta in set k
		if (!row_set_id) { 
			//crea un nuovo insieme con unico elemento l'indice della riga corrente
			tot_set++;
			dataTypeNSeq new_set_id=next_set_id;
			next_set_id++;
			reads_to_set[rowIdx%n]=new_set_id;
...
int main(int argc, char **argv) {
	...
	#elif POS
		dataTypeNSeq rem=(numRead*2)%max_rows;
		dataTypeNSeq q=(numRead*2)/max_rows; 
	#else
